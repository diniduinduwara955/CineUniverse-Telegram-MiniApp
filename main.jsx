...
import ManualMovieManager from "./ManualMovieManager.jsx";
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {active!=="Downloads"&&active!=="Settings"&&<>
        ...
        <section className="section"><div className="section-head"><h2><span className="accent-line red"/>{query.trim()?"Search Results":genreTitle?`${genreTitle} Picks`:active==="Home"?"Popular Movies":active}</h2><span className="result-count">{loading?"Loading…":`${filtered.length} titles`}</span></div><div className={query.trim()?"cu-search-results-grid":"poster-grid"}>{loading?[0,1,2,3,4,5,6,7].map(i=><div className="poster-skeleton" key={i}/> ):query.trim()?filtered.slice(0,8).map(m=><SearchResultCard key={`${m.mediaType}-${m.id}`} movie={m} onOpen={openDetails}/>):filtered.slice(0,4).map(m=><button className="poster-card large" key={`${m.mediaType}-${m.id}`} onClick={()=>openDetails(m)}><Poster movie={m}/><strong>{m.title}</strong><small>{m.year}</small></button>)}{!loading&&!filtered.length&&<div className="empty glass"><span>✦</span><strong>No titles found</strong><p>Try another search or pick a genre.</p></div>}</div></section>
