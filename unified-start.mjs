...
// Keep the existing server behavior intact, including the small Telegram payload
// compatibility guard used by the current project.
...
console.log(`[unified-bootstrap] starting backend on internal port ${BACKEND_PORT}`);
...
// Proxy the existing API to the internal backend so the Mini App can use /api
// on the exact same Render origin. No existing backend routes are changed.
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`[unified-web] Cine Universe single-service app running on port ${PORT}`);
  console.log(`[unified-web] frontend: /  | api: /api -> internal ${BACKEND_PORT}`);
});

function shutdown(signal) {
  console.log(`[unified-web] ${signal} received; shutting down backend`);
  backend.kill(signal);
  process.exit(0);
}
